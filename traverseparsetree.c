		...
		if(chi->child->next->next->next->next->next->next->next == NULL){
			...
			for(int i=l_val; i<=r_val; i++){
				...
				if(i != atoi(tptr->child->next->next->name)){
					flag = false;
					char s[1000];
					sprintf(s, "%d Index out of bound", tptr->line_no);
					strcpy(errors[errind++], s);
					temp->is_error = 1;
					break;
				}else{
					...
					if(count != size){
						flag2 = false;
						char s[1000];
						sprintf(s, "%d Definition error for declared size", tptr->line_no);
						strcpy(errors[errind++], s);
						temp->is_error = 1;
						break;
					}
			...
			TreeNode *temp_trav;
			chi = chi -> next -> next -> next -> next;			//"jagged_rows" node
			int r_val = t->jagged_3d.fd_r_index;
			int l_val = t->jagged_3d.fd_l_index;
			...
			for(int i=l_val; i<=r_val; i++){
				...
				if(i != atoi(tptr->child->next->next->name)){
					...
					while(inside3d != NULL){
						...
						prime_count++;
						TreeNode *tempinside3d = inside3d;  // list node
						//printf("\n\n%s\n\n", tempinside3d->name);
						int count = 0;
						while(tempinside3d != NULL){
		...
		switch (type_expression_table[i]._type){
			...
			case 1:
				printf("\t<type=rectangularArray,dimensions=%d,",
							type_expression_table[i].type_exp.rect.dims-1);
				dim_range* temp = type_expression_table[i].type_exp.rect.head;

				for(int j=1; j<type_expression_table[i].type_exp.rect.dims; j++){
					printf("range_R%d=(",j);
					printf("%d,%d),", atoi(temp->l_index), atoi(temp->r_index));
					temp = temp->next;
				}
				...
				break;
			case 2:
				if(type_expression_table[i].is_error != 1){
					...
					printf("Type definition error in %s\n", type_expression_table[i].name);
				}
				break;
		}
	}
